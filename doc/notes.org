...
*** Intended target user
    Physicians who perform cardiac electrophysiology procedures and
...
*** Rationale
    ...
    the CPT codes directly into the computer.  This is difficult
...
*** How does EP Coding help?
    - It lists the relevant CPT codes for different procedures, with
      succinct descriptions.
    - It makes clear which codes cannot be combined for specific procedures.
    - It analyzes selected codes for errors and warnings.
    - It provides Hints on how to code procedures.
*** How to use EP Coding
    ...
    - Codes are organized as primary codes, at least one of which
      ...
      are commonly done along with the procedure.
    - Click on the codes that are relevant.
    - Codes that shouldn't be selected or must be selected are locked.
    - Click on summarize to summarize the selections.
    - If code analysis is selected in the Settings (see below),
      Warnings and Errors will be listed including the relevant codes
      in brackets, and, if the analyzer is set to verbose, some
      further explanation.
    - Click the Clear button to clear the selected codes.  Codes are
      also cleared if you return to the procedure list or select
      another procedure.
    - Click in the Hints button for coding hints.
    - Click on the Save icon at the top to save your selection of
    ...
    - Click on the Code Analysis Settings for change the level of
      detail shown in the Code Analysis.
    - Note that the All Codes item on the list just lists every code
      in numerical order.  It can be used for 
* Limitations
